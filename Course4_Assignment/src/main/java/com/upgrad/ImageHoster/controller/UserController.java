...
public class UserController {
    ...
    public String signUpUser(@RequestParam("username") String username,
        ...
        //this method is get all the users
        User userFromDB = userService.getByName(username);
        Map<String,String> errors=validateUser(username,password,userFromDB);
        if(errors.size()>0){
       ...
       /* //to check if the user is already present
        if (userFromDB!=null&&userFromDB.getUsername() != null) {
            String usernameError = "Username already exists please change";
            ...
            String errors=validateUser(username,password);
            if(errors!=null){
...
*/
    ...
    private  Map<String,String>  validateUser(String username, String password,User userFromDB) {
        Map<String,String> errors= new HashMap<String,String>();
        if (userFromDB!=null&&userFromDB.getUsername() != null) {
            errors.put("username","Username already exists");
        }
        if(username!=null&&username.length()<6){
            errors.put("username","Username length should be minimum 6 characters");
        }
        if(password!=null&&password.length()<6){
            errors.put("password","Password length should be minimum 6 characters");
        }
        return errors;
    }
